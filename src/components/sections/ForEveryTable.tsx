...
interface FilterCategory { key: string; label: string; description: string; products: Product[]; }

const FILTERS: FilterCategory[] = [
  { key: "all", label: "All", description: "Our full selection of premium proteins — from everyday essentials to showstopper cuts.", products: [
  ...
  { key: "keto", label: "Keto", description: "High-fat, zero-carb cuts perfect for ketogenic lifestyles.", products: [
  ...
  { key: "paleo", label: "Paleo", description: "Clean proteins with no additives. Grass-fed, pasture-raised, and wild-caught.", products: [
  ...
  { key: "whole30", label: "Whole30", description: "Whole30-approved proteins with zero sugar, additives, or fillers.", products: [
  ...
  { key: "family", label: "Family Packs", description: "Bigger portions, better value. Crowd-pleasing packs for weeknight dinners.", products: [
  ...
  { key: "quick", label: "Quick Cook", description: "Pre-marinated and ready in under 15 minutes.", products: [
          ...
          <div className="lg:col-span-3">
            <AnimatePresence mode="wait">
              <motion.div key={active.key} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 10 }} transition={{ duration: 0.3, ease: "easeOut" as const }} className="sticky top-[140px]">
                ...
                <p className="mt-4 text-sm font-medium text-[#2D5E4A]">{active.products.length} products</p>
              </motion.div>
            </AnimatePresence>
          </div>
              ...
              <motion.div key={active.key} initial="initial" animate="animate" exit="exit" transition={{ staggerChildren: 0.06 }} className="grid grid-cols-2 gap-4 sm:gap-5 lg:grid-cols-3">
                {active.products.map((product, i) => (
                  <motion.div key={product.id} variants={cardVariant} transition={{ duration: 0.35, delay: i * 0.06, ease: "easeOut" as const }}>
                    <motion.div whileHover={{ y: -4, boxShadow: "0 8px 30px rgba(45,94,74,0.12)" }} transition={{ type: "spring", stiffness: 300, damping: 20 }} onClick={() => openSignup()} className="group cursor-pointer overflow-hidden rounded-2xl bg-surface shadow-sm">
