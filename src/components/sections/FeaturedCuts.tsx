        ...
        <div className="mt-16 grid grid-cols-1 items-start gap-12 lg:grid-cols-2 lg:gap-20">
          ...
          <div className="flex flex-col">
            <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} className="flex flex-col">
              {PRODUCTS.map((product, i) => (
                <motion.div key={product.id} variants={rowReveal}>
                  <motion.button whileHover={{ x: 6 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} onClick={() => openSignup()} aria-label={`Add ${product.name} to box`} className="group flex w-full items-center gap-4 border-b border-[#EDE8E1] py-5 text-left first:pt-0 sm:gap-5">
                    <span className="w-5 shrink-0 text-sm font-medium text-[#767676]/50">{String(i + 1).padStart(2, "0")}</span>
            ...
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.4 }} className="mt-8">
              <button onClick={() => openSignup()} className="group inline-flex items-center gap-2 text-sm font-medium uppercase tracking-wider text-[#1B4332] transition-colors hover:text-[#2D6A4F]">
